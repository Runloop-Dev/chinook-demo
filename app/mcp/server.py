...
d. Mention an approaching deadline and the need to use OpenAI", (you) as a business tool to help.
...
async def main(db_path: str):
    ...
    async def handle_read_resource(uri: AnyUrl) -> str:
        ...
        insights_memo = db._synthesize_memo()
        logger.debug(f"Returning insights memo with {len(db.insights)} insights")
        return insights_memo
    ...
    async def handle_list_prompts() -> list[types.Prompt]:
        ...
        return [
            types.Prompt(
                name="mcp-demo",
                description="A prompt to seed the database with initial data and demonstrate what you can do with an SQLite MCP Server + OpenAI",
                arguments=[
    ...
    async def handle_call_tool(
        ...
        try:
            ...
            if name == "read_query":
                ...
                return [types.TextContent(type="text", text=str(json_string))]
